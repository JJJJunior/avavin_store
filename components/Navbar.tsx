  ...
  return (
    <div className="sticky px-8 py-6 top-0 z-10 gap-2 flex justify-between items-center bg-white max-sm:px-2">
      ...
      <div className="flex gap-3 border border-grey-2 px-3 py-1 items-center rounded-lg">
        <input className="outline-none max-sm:max-w-[120px]" placeholder="Search..."
      ...
      <div className="relative flex gap-3 items-center">
        ...
        {dropdownMenu && (
          <div
            className="absolute top-10 right-5 flex flex-col gap-4 p-3 rounded-lg border bg-white text-base-bold lg:hidden">
            <Link href="/" className="hover:text-red-1">Home</Link>
            <Link href={user ? "/wishlist" : "/sign-in"} className="hover:text-red-1">WishList</Link>
            <Link href={user ? "/orders" : "/sign-in"} className="hover:text-red-1">Orders</Link>
            <Link
